...
def test(args):
    ...
    while not done:
        ...
        safe_radius = cbf.get_safe_radius(state[:6])
        ...
        if safe_radius - np.linalg.norm([state[0] - uni_state[0], state[1] - uni_state[1]]) > 0.015 and in_safe_set is False and cbf_on is True:
